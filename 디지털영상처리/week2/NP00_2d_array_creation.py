...
2. NumPy로 선언한 어레이의 특징
    ...
    NumPy에서는 차원(dimension)을 축(axis)라고 부르기도 한다.
        2 x 3 어레이(배열, matrix)라면;
            A=numpy.array([[1, 0, 0], [0, 1, 2]])  # 2x3 행렬(matrix) A의 선언 방법.
            1st axis : row=2, 	2nd axis : column = 3, 3rd: channel

3. 본 프로그램의 개요:
    NnumPy를 이용해 2차원 매트릭스를 정의하고, 그 매트릭스가 갖는 속성을 멤버변수를 통해 확인한다.
    np.array() 함수로 선언한다.
...
참조 링크:
    NumPy Documentation (버전 별): https://numpy.org/doc/
    최신 버전(현재 ver. 1.21.0) NumPy User Guide PDF: https://numpy.org/doc/stable/numpy-user.pdf
    Web 기반 NumPy stable version Manual: https://numpy.org/doc/stable/
    추천 quick start: https://numpy.org/doc/stable/user/quickstart.html
...
# --------------------------------------------------------------------------------------------------
# NxM Array Creation - numpy.array 함수를 이용하여 ndarray 타입의 (행x열)차원 어레이를 생성한다.
...
# 이곳에 선언된 OBJ1은 모든 단계의 테스트에 공동적으로 적용된다.
...
print(a[0][2])  # => 3    numpy는 a[0,2]로 표현해되 됨. 파이썬 리스트는 a[0][2]만 됨.
print(a[0, 2])  # =[0][2] => 3
print(a[1, 1])  # => 5
print(a[1, :-1])  # 1번 row, 처음부터 (끝-1)까지   => [4 5]
...
"""
#------------------------------------------------------------------------------------------
print("\n3) 행렬 원소의 데이터 타입을 지정하여 선언한다.")
# 지정하는 타입은 파이썬 데이터 형, numpy 데이터 형이 있다.
#------------------------------------------------------------------------------------------

# 파이썬 데이터 형은 그대로 써도 된다.  dtype = float, int, bool
b = np.array(obj1, dtype=float)     # 파이썬 데이터 형
b = np.array(obj1, dtype=int)       # 파이썬 데이터 형
b = np.array(obj1, dtype=bool)      # 파이썬 데이터 형
#b = np.array(obj1, dtype=uint8)        # NO(금지), 파이썬 형이 아니다.

# numpy 데이터형은 "타입"을 쓰던가, np.데이터형을 쓴다.
# 이하 numpy 데이터 형.
b = np.array(obj1, dtype="uint8")
b = np.array(obj1, dtype=np.uint8)
b = np.array(obj1, dtype='int')
b = np.array(obj1, dtype='float64')     # OK:'float32'
#b = np.array(obj1, dtype=np.float64)
...
"""
#-----------------------------------------------------------------------------------------
print("\n4) 내부 원소 중 하나라도 부동소수가 있으면 모든 원소가 부동소수로 정의된다.")
# 가장 규모가 큰 데이터 타임으로 정의된다.
# 왜냐하면 ndarray 데이터형은 모든 원소가 동질(homogenous)이어야 하기 때문이다.
#------------------------------------------------------------------------------------------
...
# 원래 python 자체가 64비트 이상의 정수형도 선언가능하며 이 경우도 type은 int이다.
print("\n\n실험 2: 서로 다른 형으로 어레이의 요소가 구성된 경우")
parm = ((1, 2**163), (3, 4.1))
a = np.array(parm)
print_att('a')
