...
int main(void)
{
	FILE* fp, * fq;
	int num, num1;
	fp = fopen("BOOKF.txt", "a+");
	fq = fopen("BOOKFF.txt", "a+");
	while (1)
	{
		menu();
...
void add(FILE *fp)
{
	...
	fseek(fp,0, SEEK_END);
	fprintf(fp, " %d %s %s %s", data.num, data.name, data.phone, data.address);
}
...
void update(FILE *fp, FILE *fq)
{
	...
	while (!feof(fp))
	{
		fscanf(fp, "%d %s %s %s", &data.num, &data.name, &data.phone, &data.address);
		if (strcmp(data.name, sname) == 0)
		{
			...
			printf("address"); scanf("%s", data.address);
            fseek(fq, 0, SEEK_END);
			fprintf(fq, " %d %s %s %s", data.num, data.name, data.phone, data.address);
		}
	}
...
void searchb(FILE* fq)
{
	...
	while (!feof(fq))
	{
		fscanf(fq, "%d %s %s %s", &data.num, &data.name, &data.phone, &data.address);
		if (strcmp(data.name, sname) == 0)
		{
			printf("number:%d\n", data.num);
			printf("name:%s\n", data.name);
