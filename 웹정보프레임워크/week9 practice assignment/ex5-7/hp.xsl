     ...
     <BODY> 
          ...
          <TBODY>
               <xsl:apply-templates select = "/제품/핸드폰"/>
                         ...
                         <xsl:sort order="ascending" select="가격" />
               </xsl:apply-templates>      
          </TBODY>
          </TABLE> 
          <p align="center"> 전체 <xsl:value-of select="count(//핸드폰)" /> 개의 상품이 검색되었습니다. </p>
          <p align="center"> 현재 보유하고 있는 제품의 총 판매량은 <xsl:value-of select="sum(//판매량)" /> 개입니다. </p>                
     </BODY>
     </HTML>
</xsl:template>
...
<xsl:template match = "/제품/핸드폰">
     <TR>
     <TD bgcolor="#DEE3EF"><xsl:number format="1" /></p></TD>
     <TD bgcolor="#DEE3EF"><xsl:value-of select="모델명"/></p></TD>
     <TD bgcolor="#DEE3EF"><xsl:value-of select="통신사"/></p></TD>
     <TD bgcolor="#DEE3EF"><xsl:value-of select="구입형태"/></p></TD>
     <TD bgcolor="#DEE3EF"><xsl:value-of select="제조사"/></p></TD>
     <TD bgcolor="#DEE3EF">
          <xsl:choose>
     ...
     <TD bgcolor="#DEE3EF"><xsl:value-of select="판매량"/></p></TD>
     <TD bgcolor="#DEE3EF"><xsl:value-of select="format-number(가격, '###, ###, ##0')"/></p></TD>
     </TR>
</xsl:template>
</xsl:stylesheet>