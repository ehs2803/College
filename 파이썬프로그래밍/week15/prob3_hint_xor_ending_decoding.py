...
# 정수를 입력받아 천단위로 ,가 들어간 strin을 반환한다.
def insert_comma(a):
    return f'{a:#,}'
...
#file_list = list(os.scandir(path))
print(file_list)
exit()
...
a = 12334567890
print(insert_comma(a))  # 12,334,567,890
exit()
...
# 5) 암호화된 데이터를 저장한다.
#l_data = list(coded_data)
f_out = open(encoded_fname, "wb")
f_out.write(coded_data)    # 암호화된 파일이름은 원본 앞에 _이 붙는다.
f_out.close()
