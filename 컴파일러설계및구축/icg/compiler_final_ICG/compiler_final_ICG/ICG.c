    ...
    int base, int offset, int width, int initialValue)
{
    ...
    stptr->offset = offset;
    stptr->width = width;
    stptr->iniialValue = initialValue;
...
void rv_emit(Node* ptr)
{
    ...
    if (ptr->token.number == tnumber)        // number
        ...
        if (symbolTable[stIndex].typeQualifier == CONST_TYPE) // constant
            emit1(ldc, symbolTable[stIndex].iniialValue);
            ...
            //printf("%d\n", symbolTable[stIndex].width);// simple var
...
{
    Node* root;
    char filename[20] = "perfect.mc";
    //char filename[20] = "bubble.mc";
