...
class GraphicDisplay(tk.Tk):
    ...
    def _build_canvas(self):
        ...
        canvas.create_image(450, 50, image=self.shapes[1])
        canvas.create_image(250, 150, image=self.shapes[1])
        canvas.create_image(450, 250, image=self.shapes[1])
        canvas.create_image(350, 350, image=self.shapes[1])
        canvas.create_image(550, 350, image=self.shapes[1])
        canvas.create_image(250, 450, image=self.shapes[1])
        canvas.create_image(350, 550, image=self.shapes[1])
        canvas.create_image(450, 350, image=self.shapes[2])
    ...
    def rectangle_move(self, action,x,y):
    ...
    def move_by_policy(self):
        if self.improvement_count != 0 and self.is_moving != 1:
            ...
            while len(self.agent.policy_table[x][y]) != 0:
                self.after(100,
                           self.rectangle_move(self.agent.get_action([x, y]),x,y))
                x, y = self.find_rectangle()
            self.is_moving = 0
...
class Env:
    def __init__(self):
        ...
        self.reward[3][4] = 1  # (2,2) 좌표 동그라미 위치에 보상 1
        self.reward[1][2] = -1  # (2,1) 좌표 세모 위치에 보상 -1
        self.reward[0][4] = -1  # (1,2) 좌표 세모 위치에 보상 -1
        self.reward[4][2] = -1  # (1,2) 좌표 세모 위치에 보상 -1
        self.reward[3][3] = -1  # (1,2) 좌표 세모 위치에 보상 -1
        self.reward[3][5] = -1  # (1,2) 좌표 세모 위치에 보상 -1
        self.reward[2][4] = -1  # (1,2) 좌표 세모 위치에 보상 -1
        self.reward[5][3] = -1  # (1,2) 좌표 세모 위치에 보상 -1
        self.all_state = []
