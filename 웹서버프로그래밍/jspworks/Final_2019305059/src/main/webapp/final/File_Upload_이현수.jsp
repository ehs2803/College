...
<%
    MultipartRequest multi = new MultipartRequest(
            request,
            application.getRealPath("/final/files"),   // 파일을 저장할 경로
        ...
        conn = DriverManager.getConnection(jdbc_url,"root","1234");
                stmt = conn.createStatement();
         {
            ...
            stmt.executeUpdate(String.format(
                ...
                "values ('%s', '%s', %d)",
                file1.getName(), curTime, (int)file1.length()));
            
            stmt.executeUpdate(String.format(
                    "insert into webhard (fname, ftime, fsize) " +
                    "values ('%s', '%s', %d)",
                    file2.getName(), curTime, (int)file2.length()));
            
            stmt.executeUpdate(String.format(
                    "insert into webhard (fname, ftime, fsize) " +
                    "values ('%s', '%s', %d)",
                    file3.getName(), curTime, (int)file3.length()));
